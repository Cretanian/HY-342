...
class Main 
{
    ...
    public static void createOutputFile(ArrayList<Body> myBodies, int NumberOfBodies, double RangeOfUniverse)
    {
        try 
        {
    ...
    public static int FindNumBodies(TreeNode node, ArrayList<Body> myBodies, Double X, Double Y,Double R, int flag)
    {
        int counter = 0;
        Double _X = 0.0;
        Double _Y = 0.0;
    ...
    public static TreeNode createTree(TreeNode node, ArrayList<Body> myBodies, Double X, Double Y, Double R, int flag)
    {
    ...
    {
        assert (args.length == 3): " BAD INPUT!!" ;
       
        ArrayList<Body> myBodies = new ArrayList<Body>();

        int ThredsNum = Integer.parseInt(args[2]);
        ...
        ForkJoinPool customThreadPool = new ForkJoinPool(ThredsNum);
        ...
        for(int i = 0; i < IterationNumber; ++i)
        {
            ...
            customThreadPool.submit(() -> myBodies.parallelStream().forEach( myBodie-> {assert(myBodie.getMarked() == -1) : "Input Error Body out of Range";}));

            for (int y = 0; y < myBodies.size(); ++y)
                FindForce(myBodies.get(y), Root);            
    
            customThreadPool.submit(() -> myBodies.parallelStream().forEach( myBodie-> { myBodie.newPosition(); }));
            ...
            customThreadPool.submit(() -> myBodies.parallelStream().forEach(
                myBodie-> { myBodie.reset(); })); 
        }
